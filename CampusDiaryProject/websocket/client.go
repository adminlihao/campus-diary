...
func (c *Client) ProcessData(data *SendMsg) {
	...
	if data.Type == ToOneFriend {
		...
		err := redisutil.RedisHSet(models.MESSAGE_HISTORY+c.Id, sendBuilder.String(), data.Content)
		if err != nil {
		...
		err = redisutil.RedisHSet(models.MESSAGE_HISTORY+data.ClientIDs[0], receiveBuilder.String(), data.Content)
		if err != nil {
			_ = c.Socket.WriteMessage(websocket.TextMessage, []byte("消息发送失败!"))
		}
		res, _ := redisutil.RedisHGet(models.UNREAD_CHAT+data.ClientIDs[0], c.Id)
		if res == "" {
			res = data.SendTime + "+0"
		}
		...
		for _, val := range FansList {
			...
			err = redisutil.RedisHSet(models.MESSAGE_HISTORY+toSendID, receiveBuilder.String(), data.Content)
			if err != nil {
				_ = c.Socket.WriteMessage(websocket.TextMessage, []byte("消息发送失败!"))
			}
			res, _ := redisutil.RedisHGet(models.UNREAD_CHAT+data.ClientIDs[0], c.Id)
		...
		for _, v := range data.ClientIDs {
			err = redisutil.RedisHSet(models.MESSAGE_HISTORY+v, MsgBuilder.String(), data.Content)
			if err != nil {
				_ = c.Socket.WriteMessage(websocket.TextMessage, []byte("消息发送失败!"))
			}
		}
...
func (c *Client) PullHistoryInformation() {
	HistoryInformation, err := redisutil.RedisHGetAll(models.MESSAGE_HISTORY + c.Id)
	if err != nil {
...
func ProcessNotify(notifyType int, notifyInfo []byte) {
	switch notifyType {
	case FollowNotify:
		var followNotifyInfo models.FollowNotifyInfo
		json.Unmarshal(notifyInfo, &followNotifyInfo)
		...
		//持久化通知信息
		err := redisutil.RedisZAdd(models.NOTIFY_HISTORY+"follow:"+strconv.Itoa(followNotifyInfo.ToUserID), score, data.Content)
		if err != nil {
			log.Println("关注通知持久化失败!", err)
		}
		err = redisutil.RedisHIncrBy(models.UNREAD_NOTIFY+"follow", strconv.Itoa(followNotifyInfo.ToUserID), 1)
		if err != nil {
			log.Println("关注未读通知更新失败", err)
		}
	case GiveLikeNotify:
		var giveLikeNotifyInfo models.GiveLikeNotifyInfo
		...
		score, _ := strconv.ParseFloat(data.SendTime, 64)
		//持久化通知信息
		err := redisutil.RedisZAdd(models.NOTIFY_HISTORY+"giveLike:"+strconv.Itoa(giveLikeNotifyInfo.ToUserID), score, data.Content)
		if err != nil {
			log.Println("点赞通知持久化失败!", err)
		}
		err = redisutil.RedisHIncrBy(models.UNREAD_NOTIFY+"giveLike", strconv.Itoa(giveLikeNotifyInfo.ToUserID), 1)
		if err != nil {
			log.Println("点赞未读通知更新失败", err)
		}
	case CommentNotify:
		var commentNotifyInfo models.CommentNotifyInfo
		json.Unmarshal(notifyInfo, &commentNotifyInfo)
		var content = make(map[string]interface{})
		...
		score, _ := strconv.ParseFloat(data.SendTime, 64)
		//持久化通知信息
		err := redisutil.RedisZAdd(models.NOTIFY_HISTORY+"comment:"+strconv.Itoa(commentNotifyInfo.ToUserID), score, data.Content)
		if err != nil {
			log.Println("评论通知持久化失败!", err)
		}
		err = redisutil.RedisHIncrBy(models.UNREAD_NOTIFY+"comment", strconv.Itoa(commentNotifyInfo.ToUserID), 1)
		if err != nil {
